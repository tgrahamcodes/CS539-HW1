...
class Tree(object):
    ...
    def entropy(Y):
        ...
        e = 0
        for i in Counter(Y): 
            x = - (math.log(Counter(Y)[i]/len(Y), 2) * Counter(Y)[i]/len(Y)); e = e + x
        return e
        #########################################
    ...
    def conditional_entropy(Y,X):
        ...
        #########################################
        def get_index(v,X):
              return [x for x, y in enumerate(X) if y == v]
        ce = 0.
        for x in Counter(X).keys():
            s = [Y[x] for x in get_index(x,X)]
            e = Tree.entropy(s)
            ce += e * len(s)/len(Y)
        return ce 
        #########################################
    ...
    def best_attribute(X,Y):
        ...
        '''
        #########################################
        return sorted([(Tree.information_gain(Y,X[x,:]), x) for x in range(X.shape[0])], key = lambda x:x[0], reverse = True)[0][1]
        #########################################
    ...
    def split(X,Y,i):
        ...
        for j in range(X.shape[1]):
            if X[i,j] in dict_X:
                dict_X[X[i,j]] = np.concatenate((dict_X[X[i,j]],X[:,j]), axis=1)
                dict_Y[X[i,j]] = np.append(dict_Y[X[i,j]],Y[j])
            else:
                dict_X[X[i,j]] = X[:,j]
                dict_Y[X[i,j]] = np.array([Y[j]])
        for k,v in dict_X.items():
            C[k] = Node(v.getA(),dict_Y[k])
    ...
    def most_common(Y):
        ...
        '''
        #########################################
        y = Counter(Y).most_common()[0][0]; return y
        #########################################
    ...
    def inference(t,x):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        if t.isleaf or x[t.i] not in t.C.keys(): return t.p
        else: return Tree.inference(t.C[x[t.i]],x)
        #########################################
    ...
    def load_dataset():
        ...
        '''
        #########################################
        X = np.loadtxt('data1.csv', dtype=np.str, delimiter=',')[1:,1:].T
        Y = np.loadtxt('data1.csv', dtype=np.str, delimiter=',')[1:,0]
        if (X,Y): return X,Y
        #########################################